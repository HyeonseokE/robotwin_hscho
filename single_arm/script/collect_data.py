...
def run(TASK_ENV, args):
    ...
    if args["collect_data"]:
        ...
        for episode_idx in range(st_idx, args["episode_num"]):
            ...
            # Generate skill-label visualisation PNGs
            try:
                from skill_labels.visualize import generate_skill_visualizations
                hdf5_path = os.path.join(args["save_path"], 'data', f'episode{episode_idx}.hdf5')
                output_dir = os.path.join(args["save_path"], 'skill_viz')
                generate_skill_visualizations(hdf5_path, output_dir=output_dir, episode_index=episode_idx)
            except Exception as e:
                print(f"[SkillViz] Warning: {e}")
