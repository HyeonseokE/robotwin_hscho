...
def create_hdf5_from_dict(hdf5_group, data_dict):
    for key, value in data_dict.items():
        if isinstance(value, dict):
            ...
            # Handle string lists (e.g., skill_labels string fields)
            if len(value) > 0 and isinstance(value[0], str):
                dt = h5py.special_dtype(vlen=str)
                hdf5_group.create_dataset(key, data=value, dtype=dt)
            else:
...
def concat_camera_views(head_imgs, wrist_imgs):
    """Concatenate head_camera and left_camera (wrist) side by side.
    ...
    for i in range(n):
        h_img = head_imgs[i]
        w_img = wrist_imgs[i]
        h_h, h_w = h_img.shape[:2]
        w_h, w_w = w_img.shape[:2]
        if w_h != h_h:
            scale = h_h / w_h
            new_w = int(w_w * scale)
            w_img = cv2.resize(w_img, (new_w, h_h))
        concat_frames.append(np.concatenate([h_img, w_img], axis=1))
...
def pkl_files_to_hdf5_and_video(pkl_files, hdf5_path, video_path):
    ...
    # Save concatenated head + wrist camera video
    if "left_camera" in data_list.get("observation", {}) and data_list["observation"]["left_camera"].get("rgb"):
        wrist_imgs = np.array(data_list["observation"]["left_camera"]["rgb"])
        concat_imgs = concat_camera_views(head_imgs, wrist_imgs)
        concat_video_path = video_path.replace(".mp4", "_concat.mp4")
        images_to_video(concat_imgs, out_path=concat_video_path)

    # Save observer camera (third_view) video for task monitoring
