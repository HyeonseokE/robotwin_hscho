...
from skill_labels import SkillLabelTracker
...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        # Skill-level semantic label tracker
        self._skill_label_tracker = SkillLabelTracker()
        self._skill_label_tracker.recording_enabled = self.save_data
    ...
    def get_obs(self):
        ...
        if self.data_type.get("endpose", False):
            ...
            pkl_dic["endpose"]["left_gripper"] = norm_gripper_val
        # qpos
    ...
    def _take_picture(self):  # save data
        ...
        # Capture skill-level semantic labels for this frame
        if self._skill_label_tracker.recording_enabled:
            pkl_dic["skill_labels"] = self._skill_label_tracker.capture_frame_labels(
                self.robot.get_left_arm_jointState()[:6])
    ...
    def move(
        ...
        # --- Skill label: start segment ---
        _has_skill = self._skill_label_tracker.has_pending_skill()
        if _has_skill:
            self._skill_label_tracker.start_skill_segment(
                start_joint_positions=self.robot.get_left_arm_jointState()[:6],
                start_ee_pose=np.array(self.get_arm_pose("left"), dtype=np.float32),
            )
        ...
        for left, right in zip(left_actions, right_actions):
            ...
            if (left is not None and left.action == "move") and (right is not None
                ...
                # --- Skill label: update goal ---
                if _has_skill and left.target_pose is not None:
                    self._skill_label_tracker.update_goal(goal_ee_pose=np.array(left.target_pose, dtype=np.float32))
                ...
                if self.plan_success is False:
                    if _has_skill:
                        self._skill_label_tracker.finish_skill_segment()
                    return False
                continue  # TODO
            else:
                control_seq = {
                ...
                if left is not None:
                    if left.action == "move":
                        # --- Skill label: update goal with target pose ---
                        if _has_skill and left.target_pose is not None:
                            self._skill_label_tracker.update_goal(
                                goal_ee_pose=np.array(left.target_pose, dtype=np.float32))

                        result = self.left_move_to_pose(
                            pose=left.target_pose,
                            constraint_pose=left.args.get("constraint_pose"),
                        )
                        control_seq["left_arm"] = result
                        # --- Skill label: update goal joint positions from planned path ---
                        if _has_skill and result is not None and "position" in result:
                            self._skill_label_tracker.update_goal(
                                goal_joint_positions=result["position"][-1])
                ...
                if right is not None:
                    if right.action == "move":
                        # --- Skill label: update goal with target pose (right arm) ---
                        if _has_skill and right.target_pose is not None:
                            self._skill_label_tracker.update_goal(
                                goal_ee_pose=np.array(right.target_pose, dtype=np.float32))

                        result = self.right_move_to_pose(
                            pose=right.target_pose,
                            constraint_pose=right.args.get("constraint_pose"),
                        )
                        control_seq["right_arm"] = result
                        if _has_skill and result is not None and "position" in result:
                            self._skill_label_tracker.update_goal(
                                goal_joint_positions=result["position"][-1])
    ...
    def grasp_actor(
        ...
        if not self._skill_label_tracker.has_pending_skill():
            self._skill_label_tracker.register_skill(
                skill_type="grasp", arm_tag=str(arm_tag),
                target_object_name=actor.get_name() if hasattr(actor, 'get_name') else "",
                goal_gripper_position=gripper_pos)
        if not self.plan_success:
            return None, []
    ...
    def place_actor(
        ...
        **args,
        ...
        if not self._skill_label_tracker.has_pending_skill():
            self._skill_label_tracker.register_skill(
                skill_type="place", arm_tag=str(arm_tag),
                target_object_name=actor.get_name() if hasattr(actor, 'get_name') else "",
                goal_gripper_position=1.0 if is_open else -1.0)
        if not self.plan_success:
            return None, []
        if self.need_plan:
    ...
    def move_by_displacement(
        ...
        if not self._skill_label_tracker.has_pending_skill():
            self._skill_label_tracker.register_skill(
                skill_type="move_by_displacement", arm_tag=str(arm_tag))
        if arm_tag == "left":
    ...
    def back_to_origin(self, arm_tag: ArmTag):
        if not self._skill_label_tracker.has_pending_skill():
            self._skill_label_tracker.register_skill(
                skill_type="back_to_origin", arm_tag=str(arm_tag))
        if arm_tag == "left":
