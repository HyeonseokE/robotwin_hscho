...
# 테스크가 잘 수행되었는 지 추출한 keyframe을 LLM에게 분석시키는 함수.(실패했다면, 왜 실패했는 지를 실행한 코드를 함께 제공하며 설명하도록 함.)
...
# 로봇 동작 코드에서 주요 장면 변화가 일어나는 지점에 이미지캡쳐기능을 삽입하는 함수입니다. LLM이 알아서 주요 장면 이후에 캡쳐 기능을 추가하도록 프롬프팅.
