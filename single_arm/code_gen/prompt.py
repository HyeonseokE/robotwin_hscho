...
AVAILABLE_ENV_FUNCTION = {
    ...
    "move":
    "def move(self, actions_by_arm1: tuple[ArmTag, list[Action]]).\
        Executes action sequences on the robotic arm.\
        No Return.\
        Args:\
        actions_by_arm1: Action sequence for the arm, formatted as (arm_tag, [action1, action2, ...])",
...
# This is a single-arm setup. Always use ArmTag("right"):
...
To return the arm to its initial position:
```python
arm_tag = ArmTag("right")
